...
// #define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "livnav", __VA_ARGS__)
...
bool openInputWithoutPermission(int scrWidth, int scrHeight){
	...
	//if((inputFd = suinput_open("touch_dev", &id, scrWidth, scrHeight)) == -1){
		LOGD(LOGTAG, "Cannot open device - 'qwerty'");
		return false;
	}

	__android_log_print(ANDROID_LOG_INFO, "TAG", "%d,%d,%d,%d", id.bustype, id.vendor, id.product, id.version);
